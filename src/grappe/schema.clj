...
(defn resource-exists [resource-key]
  (fn [id]
    (try
      (boolean
        (store/fetch-item deps* (resource-key (:resources-registry deps*)) {} {:find {:_id id}}))
...
;; This approach does not work, thread local bindings are not bound in the delayed context (... makes sense)
(defn map-validation [{:keys [translations config] :as deps} resource request delayed]
  (try
    @delayed
...
(defn validate-create [{:keys [hooks] :as deps} resource request payload]
  (map-validation
    deps resource request
    (delay
      (binding [deps* deps
                resource* resource
                request* request]
        (let [hooks (compose-hooks hooks resource)
              [pre-validate-fn post-validate-fn] ((juxt :pre-create-pre-validate :pre-create-post-validate) hooks)
              payload (pre-validate-fn deps resource request payload)]
          (->> (st/select-schema payload (:schema resource) cleve-coercion-matcher)
...
(defn validate-update [{:keys [hooks] :as deps} resource request payload existing]
  (map-validation
    ...
    (binding [deps* deps
              ...
              request* request]
      (let [hooks (compose-hooks hooks resource)
            [pre-validate-fn post-validate-fn] ((juxt :pre-update-pre-validate :pre-update-post-validate) hooks)
