...
(s/defrecord HTTPServer [;; construction parameters
             ...
             component/Lifecycle
             (start [this]
                    (log/info "starting http-kit server listening on " (:host config) ":" (:port config))
                    (if (:server this)
                      this
                      (assoc this :server
                                  (run-server ((wrapper (dissoc this :handler :config :server :wrapper))
                                                (wrap-dependencies handler (dissoc this :handler :config :server :wrapper)))
                                              config))))
             (stop [this]
...
(s/defn ^:always-validate new-http-server :- HTTPServer
        ...
        (component/using  (map->HTTPServer {:handler handler :wrapper wrapper :config config})
                          dependencies))
